...
def extract_page_content(url, page_type=None):
    try:
        ...
        # Return the raw HTML content instead of just the text
...
log_filename = datetime.now().strftime("scrape_log_raw_html_%Y%m%d_%H%M%S.log")
output_filename = datetime.now().strftime("scraped_content_raw_html_%Y%m%d_%H%M%S.csv")
...
with open(log_filename, 'w') as logfile:
    for index, row in df.iterrows():
        url = row['website']
        if pd.notna(url):
            # Extract main page content (raw HTML)
            main_content = extract_page_content(url)
            
            # Extract "About" page content (raw HTML)
            about_content = extract_page_content(url, 'about')
            
            # Extract "Contact" page content (raw HTML)
            ...
            # Append the raw HTML content to the list
